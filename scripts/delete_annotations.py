...
#  if "default_annotations" in mosaicInfo: defaultAnnotations(args)
#
#  # Output summary file
...
#  # Extract the json information
  ...
  if str(args.reference) != str(resourceData['reference']): fail("The selected reference (" + str(args.reference) + ") does not match the resource json reference (" + str(resourceReference) + ")")
...
def readMosaicJson(args):
  ...
  # Store information on annotationns to remove from the project
  if "remove" in mosaicData: mosaicInfo['remove'] = mosaicData['remove'] if 'remove' in mosaicData else []

# WRITE SCRIPT TO CHECK REMOVE DATA AND REMOVE
...
#  print("    --offline \\", sep = "", file = bashFile)
...
#  # If comp hets are to be ignored, the output of the de novo / recessive filter is the final output.
#  # Otherwise, use SLIVAR1 and then combine the outputs
#
...
#  for uid in mosaicInfo["default_annotations"]:
